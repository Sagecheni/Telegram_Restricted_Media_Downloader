...
class Bot:
    ...
    async def listen_info(self, client: pyrogram.Client, message: pyrogram.types):
        ...
        if not self.listen_forward_chat and not self.listen_download_chat:
            ...
            if self.listen_download_chat:
                await __listen_info(
                    self.listen_download_chat, "🕵️以下链接为已创建的`监听下载`频道:\n"
                )
            if self.listen_forward_chat:
                await __listen_info(
                    self.listen_forward_chat, "📲以下链接为已创建的`监听转发`频道:\n"
                )
    ...
    async def start_bot(
        ...
        try:
            ...
            # 监听纯 X/Twitter 链接并自动转换为 /download
            self.bot.add_handler(
                MessageHandler(
                    self.get_download_link_from_bot,
                    filters=(
            ...
            # 监听 Instagram 链接并交由 /download 逻辑处理
            self.bot.add_handler(
                MessageHandler(
                    self.get_download_link_from_bot,
                    filters=(
    ...
    def update_text(
        ...
        invalid_msg = (
            f"{BotMessage.INVALID}{n.join(sorted(invalid_link))}{n}(具体原因请前往终端查看报错信息)"
            if invalid_link
            else ""
        )
        if exist_link:
